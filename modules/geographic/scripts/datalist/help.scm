;;; Help --- Show help on datalist commands
...
;; Usage: help
;;
;; Show help for datalist scripts.

;;; Code:

(define-module (geographic scripts datalist help)
...
(define %help "
Show help on datalist commands.  With --all, show arcane incantations as
...
" (list-ref %title-list (modulo (random (current-time)) (length %title-list))))
  ...
  (for-each
   (lambda (name)
     (let* ((modname `(geographic scripts datalist ,(string->symbol name)))
            (mod (resolve-module modname #:ensure #f))
            (summary (and mod (and=> (module-variable mod '%summary)
                                     variable-ref))))
       (if (and mod
                (or all?
                    (let ((v (module-variable mod '%include-in-datalist-list)))
                      (if v (variable-ref v) #t))))
           (if summary
               (format #t "  ~A ~23t~a\n" name summary)
               (format #t "  ~A\n" name)))))
   (find-submodules '(geographic scripts datalist)))
  (format #t "
                 ...
                 (let ((var (module-variable mod '%synopsis)))
                   (if var
                       ...
                       (string-append (module-command-name mod)
                                      " OPTION...")))
                 #\newline)))
    (display "Usage: datalist " port)
    (display (car usages))
    (newline port)
    (for-each (lambda (u)
                (display "       datalist " port)
                (display u port)
                (newline port))
              (cdr usages))))
...
(define (main . args)
  (cond
   ...
   ((and (null? (cdr args)) (not (string-prefix? "-" (car args))))
    ...
    (let ((name (car args)))
      (cond
       ((resolve-module `(geographic scripts datalist ,(string->symbol name)) #:ensure #f)
        => (lambda (mod)
             (show-help mod)
             (exit 0)))
       (else
