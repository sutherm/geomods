;;; Help --- Show help on xyz commands
...
;; Usage: help
;;
;; Show help for xyz scripts.

;;; Code:

(define-module (geographic scripts xyz help)
...
(define %help "
Show help on xyz commands.  With --all, show arcane incantations as
...
" (list-ref %title-list (modulo (random (current-time)) (length %title-list))))
  ...
  (for-each
   (lambda (name)
     (let* ((modname `(geographic scripts xyz ,(string->symbol name)))
            (mod (resolve-module modname #:ensure #f))
            (summary (and mod (and=> (module-variable mod '%summary)
                                     variable-ref))))
       (if (and mod
                (or all?
                    (let ((v (module-variable mod '%include-in-xyz-list)))
                      (if v (variable-ref v) #t))))
           (if summary
               (format #t "  ~A ~23t~a\n" name summary)
               (format #t "  ~A\n" name)))))
   (find-submodules '(geographic scripts xyz)))
  (format #t "
...
(define (main . args)
  (cond
   ...
   ((and (null? (cdr args)) (not (string-prefix? "-" (car args))))
    ...
    (let ((name (car args)))
      (cond
       ((resolve-module `(geographic scripts xyz ,(string->symbol name)) #:ensure #f)
        => (lambda (mod)
             (show-help mod)
             (exit 0)))
       (else
