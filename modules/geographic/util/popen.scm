...
(define-module (geographic util popen)
  #:use-module (ice-9 threads)
  #:use-module (srfi srfi-9)
  #:export (port/pid-table open-pipe* open-pipe close-pipe open-input-pipe
	   open-output-pipe open-input-output-pipe open-input-output-pipe*))
...
If input-output mode is selected, will return a list where
the car is the read-port and the cadr is the write-port."
  (call-with-values (lambda ()
	...
	(if (pair? port)
	    (let loop ((ports port))
	      ;;(%set-port-property! (car ports) 'popen-pipe-info pipe-info)
	      (with-mutex port/pid-table-mutex
			  (hashq-set! port/pid-table (car ports) pid))
	      (if (pair? (cdr ports))
		  (loop (cdr ports))))
	    (begin
...
(define (open-input-output-pipe* command . args)
  (call-with-values
      (lambda () (apply open-process OPEN_BOTH command args))
    (lambda (read-port write-port pid)
      (let ((port (list read-port write-port))
	    (pipe-info (make-pipe-info pid)))
	...
	;; Guard the pipe-info instead of the port, so that we can still
        ...
        ;; the port entry).
	(pipe-guardian pipe-info)
        (%set-port-property! (car port) 'popen-pipe-info pipe-info)
	(%set-port-property! (cadr port) 'popen-pipe-info pipe-info)
	
        ;; XXX populate port/pid-table for backward compatibility.
        (with-mutex port/pid-table-mutex
		    (hashq-set! port/pid-table (car port) pid)
		    (hashq-set! port/pid-table (cadr port) pid))
